...
async function main() {
  ...
  try {
    ...
    if (!dbHealth.connected) {
      ...
      // Start market sync job IMMEDIATELY and then every 5 minutes
      // Run immediately on startup to populate empty database
      app.log.info("Starting IMMEDIATE market sync...");
