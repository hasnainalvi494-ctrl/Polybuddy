-- Add ALL missing elite trader columns
...
ADD COLUMN IF NOT EXISTS market_timing_score DECIMAL(5, 2),
ADD COLUMN IF NOT EXISTS risk_adjusted_returns DECIMAL(10, 4),
ADD COLUMN IF NOT EXISTS total_loss DECIMAL(18, 2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS gross_profit DECIMAL(18, 2) DEFAULT 0,
...
ADD COLUMN IF NOT EXISTS average_profit_per_trade DECIMAL(18, 2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS average_loss_per_trade DECIMAL(18, 2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS max_profit DECIMAL(18, 2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS max_loss DECIMAL(18, 2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS std_dev_returns DECIMAL(10, 4) DEFAULT 0,
ADD COLUMN IF NOT EXISTS elite_score DECIMAL(5, 2) DEFAULT 0,
...
DO $$ BEGIN
    ALTER TABLE wallet_performance ADD COLUMN trader_tier trader_tier DEFAULT 'limited';
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
    ALTER TABLE wallet_performance ADD COLUMN risk_profile risk_profile DEFAULT 'moderate';
EXCEPTION
    WHEN duplicate_column THEN null;
END $$;

-- Add category_distribution column
ALTER TABLE wallet_performance ADD COLUMN IF NOT EXISTS category_distribution JSONB;

-- Truncate existing data and insert fresh demo traders
TRUNCATE wallet_performance;

-- Insert 20 demo traders
...
-- Show summary
SELECT 
  '✅ DEMO TRADERS POPULATED!' as status,
  COUNT(*) as total_traders, 
  COUNT(*) FILTER (WHERE trader_tier = 'elite') as elite_tier,
  COUNT(*) FILTER (WHERE trader_tier = 'strong') as strong_tier,
  COUNT(*) FILTER (WHERE trader_tier = 'moderate') as moderate_tier,
  ROUND(AVG(elite_score), 2) as avg_elite_score
